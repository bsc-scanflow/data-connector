...
def find_torchserve_node(all_node_data):
    """
    Find the node that has TorchServe metrics
    """
    for node_data in all_node_data:
        if not node_data:  # Skip empty data
            continue
        if 'CPUUtilizationTS' in node_data[0] and node_data[0]['CPUUtilizationTS'] is not None:
            return node_data
    return None
...
def main(experiment_name, team_name, output_path="/workflow/data"):
    ...
    for i in range(len(instances)):
        queries = {
            ...
            "CPUUtilizationTS": (f'rate(container_cpu_usage_seconds_total{{container="torchserve", namespace="{namespaces[i]}"}}[2m])*100'),
    ...
    # Find the node with TorchServe metrics
    ...
    if source_data is not None:
        ...
        for node_data in all_node_data:
            ...
            filename = f"{node_data[0]['node']}_{node_data[0]['timestamp'].strftime('%Y%m%d_%H%M%S')}.csv"
            file_path = os.path.join(output_path, filename)
            # Save to CSV
